...
let broadlyAvailableModels = [
    ...
    { name: "o1", shortLabel: "OAI o1 preview model.", endpointUrl: "https://api.openai.com/v1/chat/completions", modelUrl: null, apiKey: "<OPENAI>", maxSequenceLength: 100000, type: "chat-completion", tokenPricing: { prompt: 0.24, completion: 0.36 } },
    { name: "o1-preview", shortLabel: "OAI o1 preview model.", endpointUrl: "https://api.openai.com/v1/chat/completions", modelUrl: null, apiKey: "<OPENAI>", maxSequenceLength: 31000, type: "chat-completion", tokenPricing: { prompt: 0.24, completion: 0.36 } },
    { name: "o1-mini", shortLabel: "OAI o1 mini preview model. ", endpointUrl: "https://api.openai.com/v1/chat/completions", modelUrl: null, apiKey: "<OPENAI>", maxSequenceLength: 64000, type: "chat-completion", tokenPricing: { prompt: 0.24, completion: 0.36 } },
    { name: "o3-mini", shortLabel: "OAI o3 mini model.", endpointUrl: "https://api.openai.com/v1/chat/completions", modelUrl: null, apiKey: "<OPENAI>", maxSequenceLength: 100000, type: "chat-completion", tokenPricing: { prompt: 0.24, completion: 0.36 } },
    ...
    { name: "gpt-4o-2024-08-06", shortLabel: "GPT 4o structured output: New ,quick, cheap, large context", endpointUrl: "https://api.openai.com/v1/chat/completions", modelUrl: null, apiKey: "<OPENAI>", maxSequenceLength: 128000, type: "chat-completion", tokenPricing: { prompt: 0.015, completion: 0.03 } },
    { name: "chatgpt-4o-latest", shortLabel: "GPT 4o public research model dynamicly updated model: New ,quick, cheap, large context", endpointUrl: "https://api.openai.com/v1/chat/completions", modelUrl: null, apiKey: "<OPENAI>", maxSequenceLength: 128000, type: "chat-completion", tokenPricing: { prompt: 0.015, completion: 0.03 } },
    ...
    { name: "gpt-4-turbo-preview", shortLabel: "GPT 4 Turbo: Latest gpt4 turbo model", endpointUrl: "https://api.openai.com/v1/chat/completions", modelUrl: null, apiKey: "<OPENAI>", maxSequenceLength: 32768, type: "chat-completion", tokenPricing: { prompt: 0.01, completion: 0.03 } },

    { name: "gpt-4.5-preview", shortLabel: "GPT 4.5 Preview: Most expensive. Costs not accurate.", endpointUrl: "https://api.openai.com/v1/chat/completions", modelUrl: null, apiKey: "<OPENAI>", maxSequenceLength: 128000, type: "chat-completion", tokenPricing: { prompt: 0.03, completion: 0.06 } },
    ...
    { name: "text-embedding-3-small", shortLabel: "text-embedding-3-small: improved version of ada-002", endpointUrl: "https://api.openai.com/v1/embeddings", modelUrl: null, apiKey: "<OPENAI>", maxSequenceLength: 1536, type: "text-embedding", tokenPricing: { prompt: 0.0004, completion: 0.0004 } },
    { name: "text-embedding-3-large", shortLabel: "text-embedding-3-large: most capable", endpointUrl: "https://api.openai.com/v1/embeddings", modelUrl: null, apiKey: "<OPENAI>", maxSequenceLength: 3072, type: "text-embedding", tokenPricing: { prompt: 0.0004, completion: 0.0004 } },
];
...
async function renderThreadList(opts = {}) {
    ...
    if (threadsWithoutCharacter.length > 0) {
        let r = prompt(`You have one or more threads (with ids=${threadsWithoutCharacter.map(t => t.id).join(",")}) that are referencing character(s) that don't exist. This is a bug. Please report it on Github or Discord. You can type "yes" below to delete these threads if a TTI dev has recommended it, otherwise just click OK.`);
...
<img src="https://ttalesinteractive.com/graphics/bin.png" width="20" height="20" alt="Delete">
    ...
    $.chatThreads.querySelectorAll(".usageStatsSpend").forEach(btn => {
        btn.addEventListener("click", async function (e) {
            e.stopPropagation();
            showError("TTI is a completely free application, but to use it, you need to connect it with one or more 'API' providers which act as the 'brains' of your characters. The spend figure you just clicked represents an estimate for the amount of money you've spent using the API that you have connected. If you're using OpenAI, you can see authorative spend amounts here:\n\nhttps://platform.openai.com/account/usage");
        });
    });
    ...
    $.characterList.querySelectorAll(".character").forEach(characterEl => {
        ...
        characterEl.querySelector(".share").addEventListener("click", async function (e) {
            ...
            if (warnThatAvatarUrlWasRemoved) {
                ...
                let result = await prompt2({
                    message: { type: "none", "html": `<p style="margin:0;">All character data is embedded within TTI share links, but this character's avatar image was stored as text (using a <a href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/Data_URLs" target="_blank">'data' URL</a>), and that would result in a huge share URL, so the avatar image was removed from the share link.<br><br>If you click 'Open avatar in new tab', then you can right-click/long-press it and save the avatar image, and then upload it to catbox.moe or a similar website, and then edit your character and replacing the 'data:' avatar URL with the new 'https:' URL that you got from the image hosting service. That way your share link will include the avatar image.</p>` },
...
async function characterDetailsPrompt(defaultValues = {}, opts = {}) {
    ...
    const result = await prompt2({
        ...
        loreBookUrlsText: { hidden: true, height: "fit-content", cssText: "white-space:pre; font-family:monospace;", label: "📖 Lorebook URLs - one per line. Use <a href='https://rentry.org' target='_blank'>rentry.org</a> or similar. Should be a url to the 'raw' text file, where your lore entries are separated by blank lines. For changes to propagate to <b>existing</b> threads, you need to use the <b>/lore</b> command and click the reload button. Visit <a href='https://ttalesinteractive.com/?page_id=2122' target='_blank'>this page</a> to learn more.", type: "text", height: "fit-content", defaultValue: loreBookUrlsText },
        customCode: { hidden: true, height: "fit-content", cssText: "white-space:pre; font-family:monospace;", label: "🧑‍💻 Custom JavaScript code. This allows you to e.g. give your bot access to the internet. Visit <a href='https://ttalesinteractive.com/?page_id=2124' target='_blank'>this page</a> to learn more.", type: "text", height: "fit-content", defaultValue: defaultValues.customCode ?? "" },
...
async function getTokenizerByModelName(modelName) {
    ...
    if (
        ...
        modelName.startsWith("o") // Added this line
    ) {
...
const openAiErrorTypeToReadableError = {
    "insufficient_quota": "Looks like you've used up all your OpenAI credits. You can buy more here:\nhttps://platform.openai.com/account\n\nNote that Elodine / TTI does not receive any money from this. OpenAI is the company that provides the 'brain' for your AI. The TTI front-end itself is completely free.",
};
...
async function getBotReply({ messages, threadId, replyingCharacter = null, replyInstruction = null, onStreamingReplyChunk, onProgressMessage, signals = {}, modelNameOverride } = {}) {
    ...
    console.log("getBotReply - messagesArr before modification:", messagesArr);

    let modelObj = availableModels[modelName];
    let apiKey = modelObj.apiKey;

    // Conditional adjustment for `o1` models

    if (modelName.startsWith("o")) {
        console.warn(`Model '${modelName}' detected. Adjusting payload for compatibility...`);
        messagesArr = messagesArr.filter(message => message.role !== "system");
        thread.temperature = 1;
        replyingCharacter.temperature = 1; // Force the temperature here too!
        delete thread.topP;
        delete thread.frequencyPenalty;
        delete thread.presencePenalty;
        console.log("Payload adjusted for 'o1' model:", { messagesArr, thread });
    }



    console.log("getBotReply - messagesArr after modification:", messagesArr);
    ...
    if (modelName.startsWith("o1")) {
        console.warn("Streaming is not supported for 'o1' models. Disabling streaming...");
        getStreamingResponse = false; // Explicitly disable streaming
    }
...
async function sendButtonClickHandler() {
  ...
  <div style="white-space: pre-wrap;"><b>Looks like this is your first thread on the TTI front-end, so here are some quick tips:</b>
    ...
    await navigator.storage.persist().then(async (persistent) => {
        if (persistent) {
            ...
            if (datesApplicationWasUsedInThisBrowser.length > 10) {
                showError("Your browser is not allowing TTI to store data in a way that is 100% permanent. Your browser may clear your data if your hard drive is nearly full. The browser will eventually grant permission once it recognises that you are a regular user of this site (i.e. once you demonstrate that you trust this site), but until then, please be sure to backup/export your data often. If you're a regular user of this front end and you're still seeing this message after a week or so of use, please submit a bug report on our Discord server.");
            }
        }
    });
} catch (e) {
